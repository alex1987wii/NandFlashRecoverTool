...
int network_listen_to(char *ip_addr, int port)
{
    ...
    int keepalive = 1; // use keepalive

    int keepidle = 20; //if there's no data trafic in 20sec, activate alive detection.

    int keepinterval = 2; // 2sec for detective interval
    int keepcount = 3; // try 3 times

    setsockopt(socket_server, SOL_SOCKET, SO_KEEPALIVE, (void *)&keepalive , sizeof(keepalive ));

    setsockopt(socket_server, SOL_TCP, TCP_KEEPIDLE, (void*)&keepidle , sizeof(keepidle ));

    setsockopt(socket_server, SOL_TCP, TCP_KEEPINTVL, (void *)&keepinterval , sizeof(keepinterval ));

    setsockopt(socket_server, SOL_TCP, TCP_KEEPCNT, (void *)&keepcount , sizeof(keepcount ));
