...
#define BB_RECOVER_REPORT_INTERVAL 5   //5sec

/*Error code for network disconnected,
error code =-31 connection reset by peer*/
#define NETWORK_ERR -31
...
static void *nand_send_inform_process(void *command)
{
    ...
    SERVER_DEBUG("DBG------>bb_recover_ack->command_id=%0X\n", bb_recover_ack->command_id);
    ...
    while(1)
    {
        ...
        /*If there is no more bad block(s) has been recovered after 6secs interval, 
        it indicatesthe recover process had finished or error occours, so terminate this thread. */
        if(tmp_bb_recover_num_new >tmp_bb_recover_num_old)
        {
            bb_recover_ack->data=tmp_bb_recover_num_new;
            /*If send error, sth wrong with the connection. Terminate this thread*/
            retval=send_to_client(bb_recover_ack); 
            if(retval<0)
                break;
    ...
    SERVER_DEBUG("DBG------>Thread: nand_send_inform_process EXIT!\n");
    return ((void *)0);
}
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        /*If retval==NETWORK_ERR, shows connection broken, 
        it's not very meaningful to do reconnect work, so 
        reboot it, device will reboot into AP mode. other err
        may be parameters error or doing BadBlock recover
        error, we can still interact with PC, leave it breath...*/
        if(retval==NETWORK_ERR)
            system("reboot");
        sleep(1);
    }
