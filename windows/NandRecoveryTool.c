...
#define WM_DOWNLOADING		(WM_USER+S_DOWNLOADING)
...
EXIT:
	if(retval != 0){/*FAILED*/
		...
		PostMessage(hwndMain,WM_DOWNLOADING,0,0);
	}
    return  (DWORD)retval;
}
...
LRESULT CALLBACK WndProc(HWND hwnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	...
	switch(message){
		...
		switch(stage){			
			...
			ERROR_MESSAGE(hwndMain,"Reboot failed! please check your device if it's connect correctly.\n");
			ClearInfoBuff();
			/*jump to S_INIT*/
			PostMessage(hwndMain,WM_INIT,1,0);
			break;
